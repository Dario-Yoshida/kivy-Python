...
class BannerVendedor(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        link = f'https://kivypython-66261-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor}"'
        requisicao = requests.get(link)
        requisicao_dic = requisicao.json()
        lista = list(requisicao_dic.values())[0]
        avatar = lista["avatar"]
        total_vendas = lista["total_vendas"]

        meu_aplicativo = App.get_running_app()

        imagem = ImageButton(source=f"icones/fotos_perfil/{avatar}",
                             pos_hint={"right":0.4, "top":0.9}, size_hint=(0.3, 0.8),
                             on_release=partial(meu_aplicativo.carregar_vendas_vendedor, lista))
        label_id = LabelButton(text=f"ID vendedor: {id_vendedor}",
                               pos_hint={"right":0.9, "top":0.9}, size_hint=(0.5, 0.5),
                               on_release=partial(meu_aplicativo.carregar_vendas_vendedor, lista))
        label_total = LabelButton(text=f"Total venda: R$ {total_vendas}",
